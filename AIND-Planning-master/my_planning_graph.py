...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        nodes = set()
        slevel = self.s_levels[level] # that is a set
        for action in self.all_actions:
            actionnode = PgNode_a(action)
            if actionnode.prenodes <= slevel:
                nodes.add(actionnode)
                for statenode in slevel:
    ...
    def add_literal_level(self, level):
        ...
        alevel = self.a_levels[level-1] # that is a set
        for actionnode in alevel:
            for statenode in actionnode.effnodes:
                nindex[statenode].append(actionnode)
            for statenode, actionnodes in nindex.items():
                for actionnode in actionnodes:
                    actionnode.children.add(statenode)
                    statenode.parents.add(actionnode)
                    nodes.add(statenode)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        a1add=set(node_a1.action.effect_add)
        a2add=set(node_a2.action.effect_add)
        a1rem=set(node_a1.action.effect_rem)
        a2rem=set(node_a2.action.effect_rem)
        return (a1add & a2rem) | (a2add & a1rem)
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        a1add=set(node_a1.action.effect_add)
        a2add=set(node_a2.action.effect_add)
        a1precond=set(node_a1.action.precond_pos+node_a1.action.precond_neg)
        a2precond=set(node_a2.action.precond_pos+node_a2.action.precond_neg)
        return  (a1add & a2precond) | (a2add & a1precond)
    ...
    def h_levelsum(self) -> int:
        ...
        for leveln, slevel in enumerate(self.s_levels):
            for statenode in slevel:
                if statenode.symbol in goals :
                    if statenode.is_pos:
                        level_sum+=leveln
                        goals.remove(statenode.symbol)
